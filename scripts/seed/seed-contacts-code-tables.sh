...
usage() {
  cat <<'USAGE'
Usage: scripts/seed/seed-contacts-code-tables.sh [options]

Seeds baseline contacts code tables for local development and environment bootstrap.
...
SEED_COMMAND="python services/inventory-backend/manage.py seed_contacts_code_tables"
...
if [ "$RUN_DOCKER" = "true" ]; then
  ...
  log_info "Seeding contacts code tables in Docker backend container"
  set -x
...
log_info "Seeding contacts code tables with local Python"
set -x
cd "$ROOT"
$SEED_COMMAND
