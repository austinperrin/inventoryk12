...
usage() {
  cat <<'USAGE'
Usage: scripts/seed/seed-organization-code-tables.sh [options]

Seeds baseline organization code tables for local development and environment bootstrap.
...
SEED_COMMAND="python services/inventory-backend/manage.py seed_organization_code_tables"
...
if [ "$RUN_DOCKER" = "true" ]; then
  ...
  log_info "Seeding organization code tables in Docker backend container"
  set -x
...
log_info "Seeding organization code tables with local Python"
set -x
cd "$ROOT"
$SEED_COMMAND
